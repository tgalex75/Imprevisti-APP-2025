...
const RegistroMercato = (props) => {
  ...
  return (
    <motion.div
      ...
      className="absolute left-1 top-1/2 hidden h-[99%] w-[20vw] -translate-y-1/2 flex-col items-center justify-between overflow-hidden rounded-lg bg-[rgb(var(--clr-bg)/.5)] pt-2 text-[--clr-txt] md:flex md:flex-col"
    >
      ...
      <ul className="flex h-full w-full flex-col gap-1 overflow-y-auto px-2 pb-2 mt-2">
        {vociRegistro.map((el) => (
          <li
            key={el.id}
            className="flex items-center justify-between bg-[rgb(var(--clr-txt)/.2)] ps-1 text-left text-[0.7rem] uppercase"
          >
